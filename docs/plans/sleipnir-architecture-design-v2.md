...
**Date:** 2026-04-22
**Revised:** 2026-04-22 (post-review v2)
**Status:** Approved (design phase) — pending final sign-off
...
// calls after all RegisterAgent calls have returned. RegisterAgent is not safe
// to interleave with Run; calling RegisterAgent after the first Run returns
// ErrHarnessFrozen. Enforce via a freeze latch set on first Run.
...
// with full-replace semantics. State is scoped per-Run per-agent within the
// runState (see §5.2). Typically passed via RunInput.ExtraTools.
...
// BufferedSink returns a Sink that buffers up to size events internally and
// a channel from which the consumer drains events. If the buffer is full,
// Send drops the event (non-blocking). Safe for concurrent use.
func BufferedSink(ctx context.Context, size int) (Sink, <-chan Event)
```
...
for i := 0; i < effective.MaxIterations; i++ {
    ...
    runState.addTokens(resp.Usage)     // sole writer to budget counter
...
| `ExtraTools` | Yes | Inherited by default (sub-agents can ask the user, write todos, etc.) |
...
type runState struct {
    // Token budget enforcement. Sole writer: the agent loop, after each
    // LLM response. TokenAccountant middleware is a pure observer — it
    // reads LLMResponse.Usage for per-agent aggregation but never touches
    // this counter.
    tokensUsed  atomic.Int64
    tokenBudget int64         // from RunInput.MaxTotalTokens; 0 = unlimited

    // Per-agent todo lists, keyed by agent name. Each sub-agent gets its
    // own list. Lists persist across iterations within a single Run but are
    // not surfaced in RunOutput (callers reconstruct from TodoEvent).
    todos sync.Map            // map[string][]TodoItem

    // Compaction watermarks, keyed by agent name. See §7 (compact middleware).
    compactState sync.Map     // map[string]*compactWatermark
}
```

**Invariant (tested):** `TokenAccountant`'s aggregate total must equal
`runState.tokensUsed` at the end of every `Run`. A divergence indicates
double-counting or a missed observation.

**Budget enforcement:**
- The loop increments `runState.tokensUsed` after each LLM response. **Sole writer.**
...
// concurrent use from multiple goroutines. Send must not block the harness
// indefinitely; implementations are responsible for their own buffering and
// drop policy. See BufferedSink for a ready-made implementation.
...
- `Send` MUST NOT block the harness indefinitely. Implementations own their
...
- `QuestionEvent` is data-only. The actual reply flow is handled by
  ...
  observers see the question regardless of handler outcome.
...
|---|---|---|
| `middleware/compact` | `Compactor` (`ContextRewriter`) | LLM-based summarization of oldest messages when prompt exceeds `CompactThreshold` (default 75% of model context). Summarizer model from `CompactModel` (default `claude-haiku-4-5`). Off for sub-agents (uses `AgentInfo.IsSubAgent`). Uses a stable compaction watermark (§7.1) to avoid re-summarizing. On failure: proceed uncompacted, emit `ErrorEvent`, let `anyllm.ErrContextLength` bubble up normally if the LLM call fails. |
| `middleware/accounting` | `TokenAccountant` (`LLMObserver`) | Pure observer: reads `LLMResponse.Usage` and aggregates per-agent into its own map. **Never touches the budget counter in `runState`.** Surfaced via `RunOutput.Usage`. Must equal `runState.tokensUsed` at run end (tested invariant). |
...
(via `runState.todos`). `AskUserTool` closes over the provided
`HITLHandler`.
...
Sub-agents inherit the `HITLHandler` from `RunInput` (§5.1).
...
- State lives in `runState.todos` (§5.2), keyed by agent name. Scoped to
  a single `Run` invocation. Each sub-agent `Run` gets its own list.
- The list persists across iterations within one `Run` (cross-turn
  planning) but is not cleared between turns.
...
| `TodoCollector` | Drains `TodoEvent`s and exposes final per-agent todo state. Convenience for tests and callers who need session-scoped todos. |
| `AssertTokenInvariant` | Asserts `TokenAccountant` aggregate equals `runState.tokensUsed` at run end. |
...
1. Module + config + `Harness.New` + freeze latch + canonical loop against
   `StubProvider` (no tools, no sub-agents).
2. Tools (`Tool` interface, `NewTypedTool`, `NewFuncTool`, collision
   validation).
3. Parallel tool dispatch + `ExtraTools` merging.
...
8. `TodoWriteTool` / `TodoReadTool` + `TodoEvent`.
9. `mcpadapter` package (snapshot semantics).
10. `sleipnirtest` helpers (`EventCollector`, `TodoCollector`, `StubTool`,
    `FakeMCPServer`).
11. Examples + CI + `go-licenses` + release process + `sleipnir.dev`
    vanity setup.
12. Urd integration: port `urd-agent` from Pydantic AI to Sleipnir;
...
Changes from original design (2026-04-22) based on architecture review:
